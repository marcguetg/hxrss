...
################################
    ...
    # imperfections of the system (from Channel_list.md document, as of 14.10.2021)
        ...
        blah = do_crystal_plot.ApplicationWindow(lpcb=self.on_crystal_map_linepicked,corrparams=self.mono2.corrparams, roi=roi)
        blah.show()
        blah.activateWindow()
    ...
    def on_crystal_map_linepicked(self,the_info):
        ...
        if the_info.valid:
            ...
            if indicate_loaded_data:
                ...
                if self.line:
                    self.line.remove()
                self.line, = the_info.ax.plot(stt_pangle_list, np.array(stt_phen_list), 'r+', gid=the_info.info_txt)
...
################################
    ...
    def on_photon_energy_enter(self):
        ...
        try:
            ...
            return
        self.loadCsv()
        self.loglabel.setText('Select a configuration from the table or click Display Map.')
    ...
    def loadCsv(self):
        ...
        enfilt =  (df['SA2 Color 1 EPH']>self.phen-1000) & (df['SA2 Color 1 EPH']<self.phen+1000)
        df = df[enfilt]
    ...
    def on_row_double_click(self):
        ...
        self.loglabel.setText('Selected configuration from '+ timestampvalue+'. Click Display Map to see reflections with this configuration.')
...
if __name__ == "__main__":
    ...
    path = os.path.join(os.path.dirname(sys.modules[__name__].__file__), 'gui/hxrss.png')
