...
from camera_gui.hxrss_camera import Ui_MainWindow
...
from hxrss_io_scanner import thread_read_worker, rt_request_update, rt_get_msg, thread_write_worker, get_initial_photon_energy_value, IO_Cmd
...
import do_crystal_plot_scanner
...
class MainWindow(qtw.QMainWindow, Ui_MainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.roll_angle_edit.returnPressed.connect(self.on_calc_roll_angle_enter)
...
################################
    ...
    # imperfections of the system (from Channel_list.md document, as of 14.10.2021)
        roi.minE = 5000
        roi.maxE = 25000   # roll angle (American convention)
        roi.phen = self.phen
        roi.minpitch = 30
        roi.maxpitch = 120
        ...
        blah = do_crystal_plot_scanner.ApplicationWindow(lpcb=self.on_crystal_map_linepicked,corrparams=self.mono2.corrparams, roi=roi)
        blah.show()
        blah.activateWindow()
    ...
    def on_crystal_map_linepicked(self,the_info):
        ...
        if the_info.valid:
            ...
            if indicate_loaded_data:
                ...
                if self.line:
                    self.line.remove()
                self.line, = the_info.ax.plot(np.array(stt_phen_list), stt_pangle_list,  'r+', gid=the_info.info_txt)
            ...
            ##    self.photon_energy_max_label.setVisible(False)
            #    self.photon_energy_max_display.setVisible(False)
            #self.calclabel.setText('Insert an energy value between '+ str(np.round(np.amin(np.array(stt_phen_list)),1)) +' eV and '+str(np.round(np.amax(np.array(stt_phen_list)),1))+' eV. Press Enter to calculate setpoint.')
...
################################
    ...
    def determine_mono_setpoints(self,mono,sp_phen):
        ...
        print(mono.infotxt+f': computing setpoint for requested photon energy {sp_phen} and roll angle {self.rollconfig}')
        ...
        #    self.calclabel.setText('Energy value between '+ str(np.round(phen_min,1)) +' eV and '+str(np.round(phen_max,1))+' eV. ')
            pass # ok
        else:
            print(mono.infotxt+f': requested photon energy is outside of possible range {phen_min}..{phen_max}')
            self.calclabel.setText('Requested reflection is outside of possible scan range.')
            self.phen = sp_phen
        ...
        self.calclabel.setText('Proposed scan pitch='+str(np.round(setpoint_pitch-1,4))+'° to '+str(np.round(setpoint_pitch+1,4))+'°.')
    ...
    def on_photon_energy_enter(self):
        ...
        try:
            ...
            self.loglabel.setText('Make sure the Photon Energy value is a valid number.')
            self.display_map_button.setEnabled(False)
            return
        self.loglabel.setText('Click Display Map and select reflection to scan around.')
